...
public class RadarView extends View {
    ...
    //默认的主题颜色
    private int DEFAULT_COLOR = getResources().getColor(R.color.radar_view_color);
    ...
    public static final int OUTER_CIRCLE_WIDTH = 7;
...
//            mTypedArray.recycle();
    ...
    private void drawCircle(Canvas canvas, int cx, int cy, int radius) {
        ...
        for (int i = 0; i < mCircleNum; i++) {
            if (i == 0) {
                mCirclePaint.setStrokeWidth(OUTER_CIRCLE_WIDTH);
            } else {
                mCirclePaint.setStrokeWidth(1);
            }
    ...
    /**
     * 生成水滴。水滴的生成是随机的，并不是每次调用都会生成同一个水滴。
     */
    private void generateRaindrop(int cx, int cy, int radius) {
        ...
        if (mRaindrops.size() < mRaindropNum) {
            ...
            if (probability) {
                ...
                if (mDegrees >= 0 && mDegrees <= 90) {
                    x = cx + xOffset;
                    y = cy - yOffset;
                } else if (mDegrees > 90 && mDegrees <= 180) {
                    x = cx + xOffset;
                    y = cy + yOffset;
                } else if (mDegrees > 180 && mDegrees <= 270) {
                    x = cx - xOffset;
                    y = cy + yOffset;
                } else {
                    x = cx - xOffset;
...
//                int xOffset = (int) (360 * Math.cos(mDegrees));
//                int yOffset = (int) (360 * Math.sin(mDegrees));
