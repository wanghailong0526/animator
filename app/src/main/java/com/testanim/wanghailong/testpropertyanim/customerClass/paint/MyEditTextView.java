...
public class MyEditTextView extends AppCompatEditText {
    ...
    private void handleCursor(int before, int j) {
        ...
        try {
            if (j + 1 < result.length()) {
                ...
                MyEditTextView.this.setSelection(result.length());
            }
        } catch (Exception e) {
    ...
    /**
     * 重写onMeasure,主要目的是让EditText的高度与我们显示在右侧的清空图标的高度相同，
     * 否则输入的时候可能会动态改变EditText的高度以适应清空图标的高度
     * 用户体验不好
     *
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthSpecMode == MeasureSpec.AT_MOST
            ...
            setMeasuredDimension(widthSpecSize, mClearDrawable.getIntrinsicHeight() + 50);
        }
    }
}
