...
//        mTouchSlop = ViewConfigurationCompat.getScaledPagingTouchSlop(configuration);//获取最小滑动距离
        mTouchSlop = configuration.getScaledTouchSlop();//获取最小滑动距离
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                intercepted = false;
//                System.out.println("whl** onInterceptTouchEvent#ACTION_DOWN:" + intercepted);
                break;
            case MotionEvent.ACTION_MOVE:
                float dx = xInterceptX - mLastInterceptX;
                float dy = yInterceptY - mLastInterceptY;
//                System.out.println("whl** onInterceptTouchEvent#ACTION_MOVE:" + intercepted);
//                System.out.println("whl** onInterceptTouchEvent#ACTION_MOVE:dy:" + Math.abs(dy));
//                System.out.println("whl** onInterceptTouchEvent#ACTION_MOVE:mLastInterceptY:" + mLastInterceptY);
//                System.out.println("whl** onInterceptTouchEvent#ACTION_MOVE:yInterceptY:" + yInterceptY);
//                System.out.println("whl** onInterceptTouchEvent#ACTION_MOVE:dx:" + Math.abs(dx));
//                System.out.println("whl** onInterceptTouchEvent#ACTION_MOVE:mTouchSlop:" + mTouchSlop);
                if (/*Math.abs(dy) > Math.abs(dx) &&*/ ev.getY() > mTouchSlop) {
            ...
            case MotionEvent.ACTION_UP:
                intercepted = false;
//                System.out.println("whl** onInterceptTouchEvent#ACTION_UP:" + intercepted);
                break;
        }
        mLastInterceptX = xInterceptX;
        mLastInterceptY = yInterceptY;
//        System.out.println("whl** intercepted:" + intercepted);
        return intercepted;
    }

    @Override
    public boolean onTouchEvent(MotionEvent event) {//处理滑动
        System.out.println("whl** onTouchEvent#down");
        if (!scrollable) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                break;
            case MotionEvent.ACTION_MOVE:
                System.out.println("whl** onTouchEvent#move");
...
//                }
//                scrollTo(0,scrollY);
                System.out.println("whl** onTouchEvent: dy= " + dy);
                System.out.println("whl** mScroller.getFinalY():" + mScroller.getFinalY());
                if (Math.abs(dy) > mTouchSlop) {
                    mScroller.startScroll(0, getScrollY(), 0, (int) dy);//mCurrY = oldScrollY + dy*scale;
                    invalidate();
                    mLastY = currY;
                }
                break;
            case MotionEvent.ACTION_UP:
//                System.out.println("whl** onTouchEvent#up");
...
//            Log.i(TAG, "onMeasure: childHeight= " + childHeight);
            ...
            if (lineWidth + childWidth > widthSize) {//换行
                if (lineViews.size() == 1 && lineViews.get(0).getLayoutParams().height == LayoutParams.MATCH_PARENT) {//如果一行只有一个元素
                    lineHeight = (int) dp2px(getContext(), 150);
                }
                views.add(lineViews);
